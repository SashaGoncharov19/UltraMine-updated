...
CI: **landed as experimental** — `build.yml` now has a `smoke-java25` job booting the same built server-dist on Temurin 25 with the modern flag set (`continue-on-error` while findings are being worked through; flip to required once it passes reliably). That job is the acceptance test for this stage.
